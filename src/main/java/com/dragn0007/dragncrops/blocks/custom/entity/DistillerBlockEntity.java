...
import com.dragn0007.dragncrops.common.gui.DistillerMenu;
...
public class DistillerBlockEntity extends BaseContainerBlockEntity implements WorldlyContainer {
   public NonNullList<ItemStack> items = NonNullList.withSize(4, ItemStack.EMPTY);
   int brewTime;
   public static final int ADD_IN_SLOT = 0;
   public static final int MASH_SLOT = 1;
   ...
   protected final ContainerData dataAccess = new ContainerData() {
      public int get(int slot) {
          if (slot == ADD_IN_SLOT || slot == MASH_SLOT) {
              return DistillerBlockEntity.this.brewTime;
          }
          return 0;
      }

      public void set(int i, int value) {
         if (i == ADD_IN_SLOT || i == MASH_SLOT) {
           DistillerBlockEntity.this.brewTime = value;
         }
      }
   ...
   public DistillerBlockEntity(BlockPos pos, BlockState state) {
      super(COBlockEntities.DISTILLER.get(), pos, state);
   }

   protected Component getDefaultName() {
      return Component.translatable("blockentities.dragncrops.distiller");
   }
   ...
   public static void serverTick(Level level, BlockPos pos, BlockState state, DistillerBlockEntity entity) {
      ...
      ItemStack grainSlotItem = entity.getItem(MASH_SLOT);
      ...
      if (entity.brewingCurrently != null && entity.brewTime > 0) {
         ...
         if (!grainSlotItem.isEmpty() && !liquidSlotItem.isEmpty()) {
            if (addInSlotItem.is(COTags.Items.APPLES)) {
               ...
               potentialOutput = COItems.BLACKBERRY_MOONSHINE.get().getDefaultInstance();
            } else if (addInSlotItem.is(COItems.BLUEBERRY.get())) {
               potentialOutput = COItems.BLUEBERRY_MOONSHINE.get().getDefaultInstance();
            } else if (addInSlotItem.is(COItems.CANTALOUPE_SLICE.get())) {
               potentialOutput = COItems.CANTALOUPE_MOONSHINE.get().getDefaultInstance();
            } else if (addInSlotItem.is(COItems.CHERRIES.get())) {
               potentialOutput = COItems.CHERRY_MOONSHINE.get().getDefaultInstance();
            } else if (addInSlotItem.is(COItems.CRANBERRY.get())) {
               potentialOutput = COItems.CRANBERRY_MOONSHINE.get().getDefaultInstance();
            } else if (addInSlotItem.is(COItems.CRIMSONBERRY.get())) {
               potentialOutput = COItems.CRIMSONBERRY_MOONSHINE.get().getDefaultInstance();
            } else if (addInSlotItem.is(COItems.GOLDENBERRIES.get())) {
               potentialOutput = COItems.GOLDENBERRY_MOONSHINE.get().getDefaultInstance();
            } else if (addInSlotItem.is(COItems.HONEYDEW_SLICE.get())) {
               potentialOutput = COItems.MELON_MOONSHINE.get().getDefaultInstance();
            } else if (addInSlotItem.is(COItems.MANGO.get())) {
               potentialOutput = COItems.MANGO_MOONSHINE.get().getDefaultInstance();
            } else if (addInSlotItem.is(COItems.PLUM.get())) {
               potentialOutput = COItems.PLUM_MOONSHINE.get().getDefaultInstance();
            } else if (addInSlotItem.is(COItems.RASPBERRY.get())) {
               potentialOutput = COItems.RASPBERRY_MOONSHINE.get().getDefaultInstance();
   ...
   public final ItemStackHandler itemHandler = new ItemStackHandler(3) {
      ...
      public boolean isItemValid(int slot, @NotNull ItemStack stack) {
         return switch (slot) {
            ...
            case MASH_SLOT -> stack.is(COItems.MASH.get());
   ...
   public AbstractContainerMenu createMenu(int pContainerId, Inventory pPlayerInventory, Player pPlayer) {
      return new DistillerMenu(pContainerId, pPlayerInventory, this, dataAccess);
   }
   ...
   protected AbstractContainerMenu createMenu(int p_58990_, Inventory p_58991_) {
      return new DistillerMenu(p_58990_, p_58991_, this, this.dataAccess);
   }
