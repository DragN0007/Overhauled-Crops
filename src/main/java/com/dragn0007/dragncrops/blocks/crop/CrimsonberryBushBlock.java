...
public class CrimsonberryBushBlock extends OBushBlock implements BonemealableBlock {
   ...
   public CrimsonberryBushBlock(Properties p_57249_) {
      super(p_57249_);
      this.registerDefaultState(this.stateDefinition.any().setValue(AGE, Integer.valueOf(0)));
   }

   public ItemStack getCloneItemStack(BlockGetter getter, BlockPos pos, BlockState state) {
      return new ItemStack(COItems.CRIMSONBERRY.get());
   }
   ...
   public InteractionResult use(BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult result) {
      ...
      if (!flag && player.getItemInHand(hand).is(Items.BONE_MEAL)) {
         ...
         popResource(level, pos, new ItemStack(COItems.CRIMSONBERRY.get(), j + (flag ? 1 : 0)));
